...
public class LiquidStone extends Item
{
    /** field for checking if the bucket has been filled. */
    private int isFull;

    public LiquidStone(int ID, int contained)
    {
        super(ID);
        this.maxStackSize = 1;
        this.setCreativeTab(TriniumRPG.tabTrinium);
    }
            ...
            if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
            {
                ...
                if (this.isFull == 0)
                {
                    ...
                    if (par2World.getBlockMaterial(i, j, k) == Material.water && par2World.getBlockMetadata(i, j, k) == 0)
                    {
                        ...
                        if (--par1ItemStack.stackSize <= 0)
                        {
                            return new ItemStack(Items.holyStone);
                        }

                        if (!par3EntityPlayer.inventory.addItemStackToInventory(new ItemStack(Items.holyStone)))
                        {
                            par3EntityPlayer.dropPlayerItem(new ItemStack(Items.holyStone.itemID, 1, 0));
                        }
    ...
    public boolean tryPlaceContainedLiquid(World par1World, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            {
                if (this.getUnlocalizedName() == Items.holyStone.getUnlocalizedName()){
                	par1World.setBlock(par2, par3, par4, Block.waterMoving.blockID);
                }
                else if (this.getUnlocalizedName() == Items.hellStone.getUnlocalizedName()){
                	par1World.setBlock(par2, par3, par4, Block.lavaMoving.blockID);
                }
                else {return false;}
            }
            else
